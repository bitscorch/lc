...
fn backtrack(
    ...
    for i in start..candidates.len() {
        ...
        if c <= remaining {
            current.push(c);
            backtrack(candidates, i, remaining - c, current, result);
            current.pop();
        }
    }
}
...
impl Solution {
    pub fn combination_sum(candidates: Vec<i32>, target: i32) -> Vec<Vec<i32>> {
        let (mut result, mut current) = (vec![], vec![]);
...
mod tests {
    ...
    #[case(vec![2], 1, vec![])]
    fn cases(#[case] candidates: Vec<i32>, #[case] target: i32, #[case] expected: Vec<Vec<i32>>) {
        assert_eq!(expected, Solution::combination_sum(candidates, target));
    }
}
