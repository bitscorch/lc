# 133. Clone Graph
# Medium | Hash Table | Depth-First Search | Breadth-First Search | Graph Theory
...
#
# Return a [**deep
# copy**](https://en.wikipedia.org/wiki/Object_copying#Deep_copy) (clone) of
# the graph.
#
...
from lc_helpers import Node, graph_of, is_deep_copy, to_adj
...
def test_cases(node, expected):
    original = graph_of(node)
    clone = Solution().cloneGraph(original)
    assert to_adj(clone) == expected
    assert is_deep_copy(original, clone)
